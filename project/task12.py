...
class TypeChecker(GrammarVisitor):
    ...
    def visitSelect(self, ctx: GrammarParser.SelectContext):
        ...
        for i in range(ctx.getChildCount()):
            if str(ctx.getChild(i)) == "where":
                reachable_ind = i
                break
        returns = [ctx.getChild(i) for i in range(left + 1, reachable_ind) if str(ctx.getChild(i)) != ',']
    ...
    def visitEdge_expr(self, ctx: GrammarParser.Edge_exprContext):
        all_types = [self.visit(ctx.expr(i)) for i in range(ctx.getChildCount()) if ctx.expr(i) != None]
...
class Interpreter(GrammarVisitor):
    ...
    def visitEdge_expr(self, ctx:GrammarParser.Edge_exprContext):  
        return (int(ctx.expr(0).getText()), ctx.expr(1).getText(), int(ctx.expr(2).getText()))

    def rsm_concatenate(self, 
        a: Regex, b: Regex
    ) -> Regex:
